...
function onFormSubmit(e) {
  ...
  try {
    ...
    // Form이 중간에 삽입한 경우 맨 끝으로 이동
    if (e && e.range) {
      const insertedRow = e.range.getRow();
      const currentLast = sheet.getLastRow();
      if (insertedRow < currentLast) {
        Logger.log(`⚠️ 잘못된 위치(${insertedRow}행) 감지 → 맨 끝으로 이동`);
        const numCols = sheet.getLastColumn();
        const rowData = sheet.getRange(insertedRow, 1, 1, numCols).getValues()[0];
        sheet.deleteRow(insertedRow);
...
/**
 * PENDING 상태이지만 메일이 발송되지 않은 행에 승인 요청 메일 재발송
...
function resendPendingEmails() {
  ...
  const targets = manager.data.filter(row => row.state === STATES.PENDING && row.identifier);

  if (targets.length === 0) {
    Logger.log("재발송 대상 없음 (PENDING + identifier 있는 행 없음)");
    return;
  }
  ...
  targets.forEach(row => {
    ...
    try {
      ...
      Logger.log(`✅ ${row.__row}행 재발송 완료`);
    } catch (err) {
      Logger.log(`❌ ${row.__row}행 재발송 실패: ${err.message}`);
    }
  });
}
